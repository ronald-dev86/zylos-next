...
export class RepositoryFactory {
  ...
  // Domain Services
  getPricingService(): PricingCalculation {
    const cacheKey = 'pricing-service'
    
    if (!this.repositories.has(cacheKey)) {
      this.repositories.set(cacheKey, new PricingCalculation())
    }
  ...
  getSalesService(): SalesService {
    const cacheKey = 'sales-service'
    
    if (!this.repositories.has(cacheKey)) {
      this.repositories.set(cacheKey, new SalesService())
    }
  ...
  getFinancialService(): FinancialService {
    const cacheKey = 'financial-service'
    
    if (!this.repositories.has(cacheKey)) {
      const ledgerRepo = this.getLedgerEntryRepository()
      this.repositories.set(cacheKey, new FinancialService(ledgerRepo))
    }
  ...
  createServicePackage() {
    ...
    return {
      ...
      pricingService: this.getPricingService(),
      salesService: this.getSalesService(),
      financialService: new FinancialService(ledgerRepository)
    }
  }

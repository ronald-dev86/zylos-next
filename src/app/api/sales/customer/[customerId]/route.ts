...
export async function GET(request: NextRequest, { params }: { params: { customerId: string } }) {
  try {
    ...
    const { page, limit } = customerSalesSchema.parse(queryParams);
    ...
    const result = await saleRepo.findByCustomerId(customerId, { page, limit });
    ...
    console.error('[API Sales customer/[customerId] GET Error]', error);

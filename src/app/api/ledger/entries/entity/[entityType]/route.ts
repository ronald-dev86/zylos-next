...
export async function GET(request: NextRequest, { params }: { params: { entityType: string } }) {
  try {
    ...
    const { page, limit } = entityTypeSchema.parse(queryParams);
    ...
    const result = await ledgerRepo.findByEntityType(entityType, { page, limit });
    ...
    console.error('[API Ledger entries entity/[entityType] GET Error]', error);
